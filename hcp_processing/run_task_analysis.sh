...
# define this for the current analysis, for the regression then the actual task
export TASK=${task[i]}
TaskNameList=""
...
#	for i in `seq 0 6`;
	if [ ! -d $working_hcp_dir"/"$subject"/"$1"_full_taskResults" ]; then
   		mkdir -p $working_hcp_dir"/"$subject"/"$1"_full_taskResults"
	fi	
	cp -r $StudyFolder"/"$subject"/MNINonLinear/Results/tfMRI_"${task[i]}"/tfMRI_"${task[i]}"_hp200_s2_level2_MSMAll.feat" $working_hcp_dir"/"$subject"/"$1"_full_taskResults/"
}
